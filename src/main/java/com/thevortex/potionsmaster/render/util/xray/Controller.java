...
public class Controller {
	...
	public static synchronized void requestBlockFinder(boolean force) {
		if (drawOres() && (task == null || task.isDone()) && (force || playerHasMoved())) // world/player check done by drawOres()
		{
			...
			WorldRegion region = new WorldRegion(lastPlayerPos, getRadius(), PotionsMaster.proxy.getClientWorld().getMinY(),PotionsMaster.proxy.getClientWorld().getMaxY()); // the region to scan for ores
